...
typedef struct grub_gui_label_ex *grub_gui_label_ex_t;

static void
label_destroy(void *vself)
{
    grub_gui_label_ex_t self = vself;
...
label_paint(void *vself, const grub_video_rect_t *region)
{
    ...
    if(self->id[0] == '$')
    {
        ...
        {
            grub_free(self->text);
            self->text = grub_strdup(g_current_input_label_info);
        }
        else if(self->id[1] == '2')
        {
            grub_free(self->text);
            self->text = grub_strdup(g_current_input_label_error);
        }
    }
    ...
    if (self->align == align_left)
        ...
        left_x = (self->bounds.width - grub_font_get_string_width(self->font, self->text)) / 2;
    else if (self->align == align_right)
        left_x = (self->bounds.width - grub_font_get_string_width(self->font, self->text));
    else
...
label_get_minimal_size(void *vself, unsigned *width, unsigned *height)
{
    ...
    *height = (grub_font_get_ascent(self->font) + grub_font_get_descent(self->font));
}
...
label_set_property(void *vself, const char *name, const char *value)
{
    ...
    else if (grub_strcmp(name, "id") == 0)
    {
        ...
        if (value)
            ...
            self->id = 0;
        if (self->id && grub_strcmp(self->id, GRUB_GFXMENU_TIMEOUT_COMPONENT_ID) == 0)
...
grub_gui_label_ex_new(void)
{
    grub_gui_label_ex_t label;
